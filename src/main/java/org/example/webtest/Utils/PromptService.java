...
public class PromptService {
    ...
    private static String securityPrompt = "###背景：在施工现场进行安全巡视，查找施工的过程中不符合安全法规的地方，来进行整改\n" +
            ...
            "}," +
            "}";
    private static String securityPromptWithRiskArea = "###背景：在施工现场进行安全巡视，查找施工的过程中不符合安全法规的地方，来进行整改\n" +
            "###角色：你是安全施工巡检员 ###要求：指出不符合的安全法规的项，要求两个版本，一个重要版，一个详细版\n"+
            "###输出内容：关注4个方面，安全隐患、重要提醒、法律依据、巡检建议，对每个方面展开多个维度的描述，并对每个维度有简短概括。针对以上内容，生成一个重要版(important_version)，一个详细版(detailed_version)，安全隐患总数量(risk_count)，并且框出每个隐患区域(rise_area)，最后对所有输出内容以json格式输出,参考输出格式如下："+
            "{" +
            "\"risk_count\": 3," +
            "\"rise_area\":\"{<x1,y1,x2,y2>,<x1,y1,x2,y2>,<x1,y1,x2,y2>}\","+
            "\"important_version\": " +
            "{\"安全隐患\": \"1. 挖掘机作业区域与工人作业区域重合，易引发机械伤害；2. 部分工人 安全帽佩戴不规范，防护不足。\","+
            "\"重要提醒\": \"挖掘机与工人近距离交叉作业，可能导致严重伤亡事故。\"," +
            "\"法律依据\":\"《中华人民共和国安全生产法》第四十二条规定，生产经营单位应当为从业人员提供符合国家标准或者行业标准的劳动防护用品，并监督、教育从业人员按照使用规则佩戴、使用。\""+
    ...
    public static String getPrompt(String sceneType) {
        String prompt = "";
        switch (sceneType) {
            case Constants.SCENE_TYPE_SECURITY:
        //        updatePromptFromFile();
                prompt = securityPrompt;
                break;
            case Constants.SCENE_TYPE_SECURITY_RISKAREA:
                prompt = securityPromptWithRiskArea;
                break;
            case Constants.SCENE_TYPE_CHAT:
                prompt = chatPrompt;
                break;
            default:
                prompt = "";
